...
[%step]
* `@NonNull` - Feld, Parameter oder Rückgabewert kann nicht `null` sein
* `@Nullable` - Feld, Parameter oder Rückgabewert kann `null` sein.
* `@NullMarked` - Alle Felder, Parameter und Rückgabewerte (an  Package, Typ, Methode oder Konstruktor) können `null` sein.
* `@NullUnmarked` - hebt `@NullMarked` auf
