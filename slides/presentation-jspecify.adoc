...
-- Tony Hoare (https://www.heise.de/hintergrund/Aus-Softwarefehlern-lernen-Teil-7-Der-Milliarden-Dollar-Fehler-11067587.html[Aus Softwarefehlern lernen – Teil 7: Der Milliarden-Dollar-Fehler]), in Bezug auf seine Einführung der Nullreferenz in der Programmiersprache ALGOL W im Jahr 1965.

[%notitle]
== Null Problemo
[%step]
Viele haben sich darüber den Kopf zerbrochen
[%step]
und _unabhängig_ voneinander gesagt:
[%step]
...
[%step]
* `@NonNull` +
Feld, Parameter oder Rückgabewert kann nicht `null` sein
* `@Nullable` +
Feld, Parameter oder Rückgabewert kann `null` sein.
* `@NullMarked` +
Alle Felder, Parameter und Rückgabewerte (an  Package, Typ, Methode oder Konstruktor) können nicht `null` sein.
* `@NullUnmarked` +
hebt `@NullMarked` auf
...
[source,java]
----
// Die Elemente in der Liste können null sein
List<@Nullable String> tags;
----
[%step]
[source,java]
----
// Liste und Elemente in der Liste können null sein
@Nullable
List<@Nullable String> tags;
----
...
[source,java]
----
// Die Elemente im Array können null sein
String @Nullable[] tags;
----
[%step]
[source,java]
----
// Array und Elemente im Array können null sein
@Nullable
String @Nullable[] tags;
----

== 📏 Statische Codeanalyse in der IDE

== 📐 Statische Codeanalyse beim Build
[%step]
mit
...
* _Spring nullability +
https://github.com/spring-gradle-plugins/nullability-plugin_
...
== 🌶️ Lombok
[%step]
Lombok kann `@NonNull` und `@Nullable` in den generierten Lombok generierten Code einfügen.
...
* Spring Blog: Null-Safe applications with Spring Boot 4 +
